...
const
  Exclusao = 'Deseja excluir o registro selecionado?';
...
type
  ...
  private
    ...
    function TraduzMensagem(E: Exception): String;
  protected
...
resourcestring
  RsAppError = '%s - erro na aplicação';
  RsExceptionClass = 'Excessão classe: %s';
  RsExceptionAddr = 'Excessão endereço: %p';
  RsStackList = 'O empilhamento da lista gerado %s';
  RsModulesList = 'Lista de módulos carregados:';
  RsOSVersion = 'Sistema   : %s %s, Versão: %d.%d, Build: %x, "%s"';
  RsProcessor = 'Processador: %s, %s, %d MHz %s%s';
  RsScreenRes = 'Display  : %dx%d pixels, %d bpp';
  RsActiveControl = 'Hierarquia de Controles ativos:';
  RsThread = 'Thread: %s';
  RsMissingVersionInfo = '(Sem informações de versão)';
...
begin
  ...
  try
    ...
    begin
      ...
      TextLabel.Text := AdjustLineBreaks(StrEnsureSuffix('.', TraduzMensagem(E)));
...
function TExceptionDialog.TraduzMensagem(E: Exception): String;
begin
  {willimar}
  {esta função faz a tradução da mensagem de erro.}
  if E.ClassName = 'EZSQLException' then begin
    if (Pos('t create database', E.Message) > 0) and (Pos('Database exists', E.Message) > 0) then begin
      Result := 'Não foi possivel criar a base de dados. A base de dados já existe.';
      exit;
    end
    else if (Pos('SQL Error: Duplicate entry', E.Message) > 0) then begin
      Result := 'Tentativa de duplicar um registro bloqueada pelo sistema.';
      exit;
    end
    else if (Pos('SQL Error: Unknown database', E.Message) > 0) then begin
      Result := 'Não foi possivel encontrar a base de dados ' + QuotedStr('mclocadora') + ', é possivel que o banco de dados esteja fechado ou a base de dados não exista.';
      exit;
    end
    else if (Pos('SQL Error: Column', E.Message) > 0) and (Pos('cannot be null.', E.Message) > 0) then begin
      Result := 'Tentativa de corromper a integridade detectada e bloqueada.';
      exit;
    end;
  end
  else if E.ClassName = 'EDatabaseError' then begin
    if (Pos('annot perform this operation on a closed dataset', E.Message) > 0) then begin
      Result := 'Não foi possivel efetuar a operação, pois a tabelas estava fechada.';
      exit;
    end
    else if (Pos('Cannot access field', E.Message) > 0) then begin
      Result := 'O sistema não conseguiu acessar o campo com este tipo de valor.';
      exit;
    end
    else if (Pos('Row data is not available', E.Message) > 0) then begin
      Result := 'Os dados ainda não estavam disponíveis.';
      exit;
    end;
  end
  else if E.ClassName = 'EConvertError' then begin
    if (Pos('is not a valid floating point value', E.Message) > 0) then begin
      Result := 'Este não é um número válido.';
      exit;
    end
    else if (Pos('is not a valid integer value', E.Message) > 0) then begin
      Result := (Copy(E.Message, 1, Pos('''', Copy(E.message, 2, length(E.message))))) + ' não é um número inteiro válido.';
      exit;
    end
    else if (Pos('is not a valid date', E.Message) > 0) then begin
      Result := (Copy(E.Message, 1, 12) + ' não é uma data válida.');
      exit;
    end;
  end
  else if E.ClassName = 'EListError' then begin
    if (Pos('List index out of bounds', E.Message) > 0 ) then begin
      Result := 'O tamanho da lista é -1.';
      exit;
    end;
  end
  else if E.ClassName = 'EDBClient' then begin
    if (Pos('At beginning of table', E.Message) > 0 ) then begin
      Result := '.';
      exit;
    end;
  end
  else if E.ClassName = 'EAccessViolation' then begin
    if (Pos('Access violation at address', E.Message) > 0 ) then begin
      Result := 'Violação de acesso a memória no endereço ' +
                Copy(E.Message, 29, 8) + ' do módulo ' +
                DM.SisControl.ApplicationNome + '. Lendo o endereço ' +
                Copy(E.Message, length(E.Message) - 7, 9);
      exit;
    end;
  end
  else if E.ClassName = 'EInvalidOperation' then begin
    if (Pos('Cannot focus a disabled or invisible window', E.Message) > 0 ) then begin
      Result := 'O sistema não pode colocar o foco em uma janela que está invisivel ou desabilitada.';
      exit;
    end;
  end
  else if E.ClassName = 'EZDatabaseError' then begin
    if (Pos('SQL Error:  violation of FOREIGN KEY constraint', E.Message) > 0) AND
       (Pos('Error Code: -530. violation of FOREIGN KEY constraint', E.Message) > 0) then begin
      Result := 'Não foi possivel excluir o registro selecionado. Provavelmente este registro está sendo usado em outros lugares.'#13'Caso realmente deseje excluir este registro exclua antes os registros que estejam usando ele.';
      exit;
    end
    else if (Pos('PRIMARY or UNIQUE KEY', E.Message) > 0) then begin
      Result := 'Tentativa de violar a chave.'#13'Registro não pode ser duplicado, provavelmente um dos valores de identificação como ''Código'' ou em alguns casos o próprio ''Nome'' foi informado e já consta cadastrado no sistema.';
      exit;
    end;
  end;
  Result := E.message;
